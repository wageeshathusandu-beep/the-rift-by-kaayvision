...
export default function About() {
  ...
  return (
    <section id="about" className="py-28 md:py-36 relative overflow-hidden" ref={ref}>
      ...
      {/* Ambient glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[500px] h-[300px] bg-rift-blood/[0.04] rounded-full blur-[130px]"></div>
      <div className="absolute bottom-1/3 right-0 w-[300px] h-[300px] bg-rift-neon/[0.025] rounded-full blur-[100px]"></div>
      ...
      <div className={`relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className={`text-center mb-10 transition-all duration-[1.2s] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <div className="flex items-center justify-center gap-4 mb-4">
            <div className="h-[1px] w-12 md:w-20 bg-gradient-to-r from-transparent to-rift-blood/40"></div>
            <div className="w-2 h-2 rotate-45 border border-rift-blood/40"></div>
            <div className="h-[1px] w-12 md:w-20 bg-gradient-to-l from-transparent to-rift-blood/40"></div>
          </div>
          <h3 className="font-cinematic font-black text-2xl md:text-3xl lg:text-4xl uppercase tracking-[0.2em] text-white/90" style={{ textShadow: '0 0 12px rgba(139,0,0,0.4), 0 0 30px rgba(139,0,0,0.15), 0 0 60px rgba(0,212,255,0.08)' }}>
            A Sisara Sanal Film
          </h3>
          <div className="flex items-center justify-center gap-4 mt-4">
            <div className="h-[1px] w-12 md:w-20 bg-gradient-to-r from-transparent to-rift-neon/25"></div>
            <div className="w-2 h-2 rotate-45 border border-rift-neon/30"></div>
            <div className="h-[1px] w-12 md:w-20 bg-gradient-to-l from-transparent to-rift-neon/25"></div>
          </div>
        </div>

        {/* Section intro */}
        <div className="text-center mb-4">
          <p className="text-rift-steel/70 text-[10px] font-cinematic tracking-[0.4em] uppercase mb-3">The Story</p>
        </div>
        <h2 className="section-heading">About The Film</h2>
        <p className="text-center text-gray-400 mb-14 max-w-xl mx-auto font-body text-lg tracking-wide">A new era of AI horror cinema</p>

        <div className="grid md:grid-cols-2 gap-12 items-center">
          {/* Left - Image placeholder */}
          <div className={`glass-card p-2 overflow-hidden group transition-all duration-700 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`} style={{ transitionDelay: '0.2s' }}>
            <div className="w-full h-80 md:h-96 bg-gradient-to-br from-[#060810] to-[#020204] rounded-lg flex items-center justify-center border border-rift-neon/[0.05] relative overflow-hidden">
              {/* HUD grid */}
              <div className="absolute inset-0 opacity-[0.02]" style={{ backgroundImage: 'linear-gradient(rgba(0,212,255,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(0,212,255,0.4) 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>
              <div className="text-center relative z-10">
                <svg className="w-16 h-16 mx-auto text-rift-steel/30 mb-3 group-hover:text-rift-neon/40 transition-colors duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0022.5 18.75V5.25A2.25 2.25 0 0020.25 3H3.75A2.25 2.25 0 001.5 5.25v13.5A2.25 2.25 0 003.75 21z" />
                </svg>
                <p className="text-rift-steel/40 text-sm tracking-[0.2em] font-nav uppercase">Scene Still</p>
              </div>
            </div>
          </div>
          ...
          <div className={`space-y-6 transition-all duration-700 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`} style={{ transitionDelay: '0.4s' }}>
            ...
            <blockquote className="border-l-2 border-rift-neon/25 pl-4 mt-6">
              <p className="text-gray-400 italic font-body text-sm md:text-base tracking-wide">
                "Some doors were never meant to be opened. Some intelligences were never meant to awaken."
              </p>
            </blockquote>

            <div className="grid grid-cols-2 gap-4 pt-4">
              <div className="glass-card p-4 text-center group/stat">
