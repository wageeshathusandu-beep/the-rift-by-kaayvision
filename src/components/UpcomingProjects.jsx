...
export default function UpcomingProjects() {
  ...
  return (
    <section id="upcoming" className="py-24 md:py-32 relative overflow-hidden" ref={ref}>
      ...
      {/* Ambient orbs */}
      <div className="orb-cyan w-[350px] h-[350px] top-[10%] left-[-5%] opacity-25"></div>
      <div className="orb-red w-[300px] h-[300px] bottom-[15%] right-[-8%] opacity-30"></div>

      <span className="absolute top-6 left-4 md:left-8 tech-label">ARCHIVE FILE 006 // FUTURE OPS</span>

      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`text-center mb-14 transition-all duration-[1.2s] ease-smooth ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 mb-14 stagger-children ${isVisible ? 'revealed' : ''}`}>
          {projects.map((p) => (
            <div key={p.id} className="ind-card p-8 text-center group">
              <p className="tech-label mb-3">OP-{p.id}</p>
              <span className="text-5xl font-display text-rift-metal/10 group-hover:text-rift-amber/10 transition-colors duration-500">{p.id}</span>
              <h3 className="font-display text-xl tracking-[0.1em] text-rift-offwhite mt-2 mb-3 group-hover:text-white transition-colors duration-300">{p.title}</h3>
              <div className="flex items-center justify-center gap-2">
                <span className={`w-1.5 h-1.5 rounded-full transition-shadow duration-500 ${p.id === '01' ? 'bg-rift-red group-hover:shadow-red-glow' : p.id === '02' ? 'bg-rift-amber group-hover:shadow-amber-glow' : 'bg-rift-metal'}`}></span>
        ...
        <div className={`text-center mb-8 transition-all duration-[1s] ease-smooth ${isVisible ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: '0.5s' }}>
