  ...
  return (
    <section id="cast" className="py-24 relative" ref={ref}>
      ...
      <div className={`relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <p className="text-center text-gray-400 mb-16 max-w-xl mx-auto">The visionaries behind The Rift</p>

        {/* Cast */}
        <h3 className="text-xl font-cinematic font-semibold text-white mb-8 text-center tracking-wider">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-16">
          {castMembers.map((member, index) => (
            ...
            >
              ...
              <p className="text-rift-neon text-xs md:text-sm mt-1">{member.role}</p>
              <p className="text-gray-500 text-xs mt-1">{member.type}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {crewMembers.map((member) => (
            <div key={member.name} className="glass-card p-4 text-center hover:border-rift-neon-purple/30 transition-all duration-300">
              ...
              <p className="text-rift-neon-purple text-xs mt-1">{member.role}</p>
            </div>
          ))}
        </div>
      </div>
